...
class KPConvFPN(nn.Module):
    """Four-stage GeoTransformer KPConvFPN for the PointCT Point branch.

    The returned list is ordered fine-to-coarse and has channel dimensions
    ``[output_dim, init_dim * 8, init_dim * 16]``. With the frozen M2-1
    configuration this is ``[256, 512, 1024]``.
    """
    ...
    def forward(self, feats, data_dict):
        ...
        feats_s1 = self.encoder1_1(feats, points_list[0], points_list[0], neighbors_list[0])
        ...
        latent_s3 = self.decoder3(torch.cat([latent_s3, feats_s3], dim=1))

        latent_s2 = nearest_upsample(latent_s3, upsampling_list[1])
        latent_s2 = self.decoder2(torch.cat([latent_s2, feats_s2], dim=1))

        return [latent_s2, latent_s3, feats_s4]
